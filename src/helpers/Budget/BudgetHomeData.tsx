...
export default function BudgetHomeData({
  budgets,
  addBudget,
  editBudget,
  ...
  const budgetDisplayData = budgets.map((budget) => {
    return (
      ...
      >
        ...
        <td className="border-x-2 p-3 text-right">
          {budget.to_spend_cost !== null ? budget.to_spend_cost.toFixed(2) : ""}
        </td>
        <td className="border-x-2 p-3 text-right">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-base relative w-full">
            <form onSubmit={handleSubmit}>
              <SearchBar onChange={searchBarChange} value={searchTerms} />
            </form>
          </div>
          <div className="text-base text-right w-1/4">
            <AddButton buttonType="button" text="Add" onEvent={addBudget} />
          </div>
        </div>
      </caption>
